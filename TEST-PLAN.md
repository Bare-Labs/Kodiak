...
**Purpose:** Validate Kodiak MCP server from the perspective of a financial analyst/manager whose AI agent uses the MCP. Execute workflows via MCP tools; record results for regression and for agents reworking features or developing new requirements.

**Prerequisites:** Python 3.11+, MCP config (e.g. Cursor with Kodiak MCP), optional Alpaca paper keys, optional CSV data in `HISTORICAL_DATA_DIR`.
...
Environment: paper broker (Alpaca), API keys configured. CSV: no (HISTORICAL_DATA_DIR not set / data dir not found). MCP client: Cursor Kodiak MCP; A7/B5 verified via server import where tool not in client.
...
Environment: paper broker (Alpaca), API keys configured. CSV: no (HISTORICAL_DATA_DIR not set / data dir not found). MCP client: Cursor Kodiak MCP. **Note**: MCP server running old code; requires restart to pick up fixes. Direct app layer tests confirm fixes work.
