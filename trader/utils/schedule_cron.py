...
AUTOTRADER_MARKER = "# Kodiak schedule"
...
def is_schedule_enabled() -> bool:
    """Return True if the Kodiak cron job is present."""
...
def enable_schedule(every_minutes: int = 5) -> None:
    ...
    lines = get_current_crontab()
    # Remove any existing Kodiak line so we don't duplicate
...
def disable_schedule() -> None:
    """Remove the Kodiak cron job if present.
